...
router.post('/api/addUpdateProduct',(req, res) => { 
  console.log("PRODUCT     .............. ", req.body);
  
  let productForMongo = new productModel({
  ...
  productModel.findOne({productName: req.body.productName}, (err, product) => {
        if (err){
            //console.log("got an error!");            
            res.send(err);
        }
        if (!product) {
          //console.log("No Student Present, Adding!"); 
          productForMongo.save((err, data, next)=>{        
            if (err) {
                res.send("Error Occurred"+ err);
            }      
            res.json(data);
...
router.get('/api/products_NEW',(req, res) =>{
  console.log("getting new PRODUCTS     .............. ", req.body);
...
MongoClient.connect("mongodb://localhost:27017/", function(err, db) {
  ...
  if (err) throw err;
  var dbo = db.db("mernmongodb");

  // products_NEW = dbo.collection("products").find({}).toArray();

    // ??????? DOESNT WORK
    // var products_NEW = dbo.collection("products").find({}).toArray();

  dbo.collection("products").find({}).toArray(
     function(err, products_NEW) {
      ...
      if (err) throw err;
        ...
        setTimeout(()=> {res.json(products_NEW);}, 100);

      });
});
console.log("  DONE                   -----------------              products_NEW");
console.log(products_NEW);

// setTimeout(()=> {res.json(products_NEW);}, 100);
//setTimeout(()=> {res.json(products);}, 100);
